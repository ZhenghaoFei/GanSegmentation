...
b_real = tf.placeholder(tf.float32, shape=[None, load_size[0], load_size[1], 3])

ab_match_pair = tf.placeholder(tf.float32, shape=[None, load_size[0], load_size[1], 6])
ab_unmatch_pair = tf.placeholder(tf.float32, shape=[None, load_size[0], load_size[1], 6])
ab_generator_pair = tf.placeholder(tf.float32, shape=[None, load_size[0], load_size[1], 6])
...
try:
    ...
    for it in range(sess.run(it_cnt), max_it):
        ...
        if (it + 1) % 100 == 0:
            ...
            a2b_opt = sess.run(a2b, feed_dict={a_real: a_real_ipt})
            sample_opt = np.concatenate((a_real_ipt, a2b_opt), axis=0)

            save_dir = './outputs/sample_images_while_training/' + dataset + saveidx
            utils.mkdir(save_dir)
            im.imwrite(im.immerge(sample_opt, 1, 2), '%s/Epoch_(%d)_(%dof%d).jpg' % (save_dir, epoch, it_epoch, batch_epoch))
except:
    print("ERROR")
    save_path = saver.save(sess, '%s/Epoch_(%d)_(%dof%d).ckpt' % (ckpt_dir, epoch, it_epoch, batch_epoch))
    print('Model saved in file: % s' % save_path)
