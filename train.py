...
ab_pair_test_pool = data.ImageDataPair(sess, a_test_img_paths, batch_size=len(a_test_img_paths), load_size=load_size, shuffle=False, crop_size=crop_size)
...
for it in range(sess.run(it_cnt), max_it):
    ...
    # train G
    g_summary_opt, _ = sess.run([g_summary, g_train_op], feed_dict={a_real: ab_match_pair_ipt[:,:,:,:3], 
                                                                    b_real: ab_match_pair_ipt[:,:,:,3:4]})
    ...
    if (it + 1) % 100 == 0:
        ...
        eval_summary_opt, predict  = sess.run([eval_summary, a2b], 
                                        feed_dict={b_real: b_ipt, 
                                                   a_real: a_ipt,
                                                   })

        summary_writer.add_summary(eval_summary_opt, it)


        # predict = sess.run(a2b, feed_dict={a_real: a_ipt})
