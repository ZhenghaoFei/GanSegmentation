...
parser.add_argument('--saveidx', dest='saveidx', default='hiddenlayer', help='appendix')
...
""" graph """
# models
generator_a2b = partial(models.generator4, scope='a2b')
...
# b_fake = tf.placeholder(tf.float32, shape=[None, crop_size, crop_size, 4])

a2b_sample = tf.placeholder(tf.float32, shape=[None, crop_size, crop_size, 3])
b2a_sample = tf.placeholder(tf.float32, shape=[None, crop_size, crop_size, 4])
...
a2b_logit = discriminator_b(a2b[:, :, :, :3])
...
g_loss = g_loss_a2b + cyc_loss_a * 10.0 
...
d_summary_a = utils.summary({d_loss_a: 'd_loss_a(invalid)'})
...
print('iter: ', sess.run(it_cnt), max_it)
for it in range(sess.run(it_cnt), max_it):
    ...
    a2b_opt = sess.run(a2b[:,:,:,:3], feed_dict={a_real: a_real_ipt, b_real: b_real_ipt})
    ...
    if (it + 1) % 100 == 0:
        ...
        [a2b_opt, a2b2a_opt] = sess.run([a2b, a2b2a], feed_dict={a_real: a_real_ipt, b_real: b_real_ipt})
        sample_opt = np.concatenate((a_real_ipt, a2b_opt[:,:,:,:3], a2b2a_opt, b_real_ipt), axis=0)
